...
def get_task(current_user, request=None, task_master_id=None):
    ...
    task = Tasks.query.join(Task_Master).filter(
        Tasks.user_id==current_user.id, Task_Master.task_type==task_blueprint.task_type, Tasks.status==-1).first()
...
def form_task(task_blueprint, task, request):
    ...
    if task.task_id == 1:
        ...
        form.question.label = "What is this: " + eval(task_blueprint.task_type).query.filter_by(id=task.answer).first().romanji
        form.question_english.label = ""
        form.question_lang.label = "ja"
        form.mc.choices = [(task.place1, eval(task_blueprint.task_type).query.filter_by(id=task.place1).first().kana),
        ...
        form = Multichoice(request.form)
        form.question.label = "What is this: " + eval(task_blueprint.task_type).query.filter_by(id=task.answer).first().kana
        form.question_english.label = ""
        form.question_lang.label = "ja"
        form.mc.choices = [(task.place1, eval(task_blueprint.task_type).query.filter_by(id=task.place1).first().romanji),
        ...
        form = Multichoice(request.form)
        form.question.label = "What is this: " + eval(task_blueprint.task_type).query.filter_by(id=task.answer).first().english
        form.question_english.label = ""
        form.question_lang.label = "ja"
        form.mc.choices = [(task.place1, eval(task_blueprint.task_type).query.filter_by(id=task.place1).first().kana),
                           (task.place2, eval(task_blueprint.task_type).query.filter_by(id=task.place2).first().kana),
        ...
        form.question_english.label = ""
        form.question_lang.label = "ja"
        form.mc.choices = [(task.place1, eval(task_blueprint.task_type).query.filter_by(id=task.place1).first().english),
...
def submit_task(current_user, answer_id_list=None, task_type=None):
    ...
    task = Tasks.query.join(Task_Master).filter(
        Tasks.user_id==current_user.id, Task_Master.task_type==task_type, Tasks.status==-1).first()
    ...
    if correct:
        if known:
            ...
            if task_type=="Kana":
                new_known = Kana_Known(user_id=current_user.id, kana_id=task.answer,  
                 level=1, last_practiced=datetime.now(), tokens=1)
            elif task_type=="Words":
               new_known = Words_Known(user_id=current_user.id, word_id=task.answer,  
                 level=1, last_practiced=datetime.now(), tokens=1)
            elif task_type=="Needs":
               new_known = Needs_Known(user_id=current_user.id, english=task.answer, japanese=task.answer,
                 level=1, last_practiced=datetime.now(), tokens=1)
        ...
        if known:
            ...
            if task_type=="Kana":
                new_known = Kana_Known(user_id=current_user.id, kana_id=task.answer,  
                 level=0, last_practiced=datetime.now(), tokens=0)
            elif task_type=="Words":
               new_known = Words_Known(user_id=current_user.id, word_id=task.answer,  
                 level=0, last_practiced=datetime.now(), tokens=0)
            elif task_type=="Needs":
...
def make_task(current_user, task_blueprint):
    ...
    if task_blueprint.task_type=="Kana":
        ...
        df = pd.read_sql('SELECT user_id, kana_id, level from "Kana_Known" WHERE user_id=\'' + str(current_user.id) + '\'', db.engine.connect().connection)
    elif task_blueprint.task_type=="Words":
        task_type_id = "word_id"
        df = pd.read_sql('SELECT user_id, word_id, level from "Words_Known" WHERE user_id=\'' + str(current_user.id) + '\'', db.engine.connect().connection)
    elif task_blueprint.task_type=="Needs":
        task_type_id = "need_id"
        df = pd.read_sql('SELECT user_id, need_id, level from "Needs_Known" WHERE user_id=\'' + str(current_user.id) + '\'', db.engine.connect().connection)
    ...
    if not answer or random.random()<0.1:
        ...
        if task_blueprint.task_type=="Needs":
            ...
            words = pd.read_sql('SELECT * from "Words_Known" WHERE user_id=\'' + str(current_user.id) + '\'', db.engine.connect().connection)
            ...
            if task_blueprint.id==13 or task_blueprint.id==16:
                task = Tasks(user_id = current_user.id, task_id = task_blueprint.id, answer = answer.english,
                place1 = romanji_from_kana(answer.japanese), status = -1, task_made = datetime.now(), task_ended = None)
            elif task_blueprint.id==15:
                task = Tasks(user_id = current_user.id, task_id = task_blueprint.id, answer = answer.japanese,
                place1 = romanji_from_kana(answer.japanese), status = -1, task_made = datetime.now(), task_ended = None)
            else:
                task = Tasks(user_id = current_user.id, task_id = task_blueprint.id, answer = answer.japanese,
                place1 = answer.english, status = -1, task_made = datetime.now(), task_ended = None)
            ...
            if df.shape[0]>=16:
                ...
                tags_select = ['Hiragana_Basic','Hiragana_Basic','Hiragana_Basic','Hiragana_Basic','Hiragana_Basic',
                               'Hiragana_Medium','Hiragana_Medium','Hiragana_Medium','Hiragana_Hard',
                               'Katagana_Basic','Katagana_Basic','Katagana_Basic','Katagana_Basic',
                               'Katagana_Medium','Katagana_Medium','Katagana_Hard']

                answer = random.choice(eval(task_blueprint.task_type).query.filter_by(tags=random.choice(tags_select)).all())
            else:
                answer = random.choice(eval(task_blueprint.task_type).query.filter_by(tags='Hiragana_Basic').all())

    question_set = [answer]

    df1 = pd.read_sql('SELECT task_id, answer, response from "Tasks" WHERE task_id=\'' + str(task_blueprint.id) + '\' AND answer!=response AND answer=\''+ str(answer.id) + "'", db.engine.connect().connection)
    df2 = pd.read_sql('SELECT task_id, response as answer, answer as response from "Tasks" WHERE task_id=\'' + str(task_blueprint.id) + '\' AND answer!=response AND response=\''+ str(answer.id) + "'", db.engine.connect().connection)
    df = pd.concat([df1,df2], axis=0)
    df = df[df["answer"]!="-1"]
    df = df[df["response"]!="-1"]
    everyone_df = df["response"].value_counts()/df["response"].count()

    df1 = pd.read_sql('SELECT answer, response, user_id from "Tasks" WHERE task_id=\'' + str(task_blueprint.id) + '\' AND user_id=\'' + str(current_user.id) + '\' AND answer!=response AND answer=\''+ str(answer.id) + "'", db.engine.connect().connection)
    df2 = pd.read_sql('SELECT response as answer, answer as response, user_id from "Tasks" WHERE task_id=\'' + str(task_blueprint.id) + '\' AND user_id=\'' + str(current_user.id) + '\' AND answer!=response AND response=\''+ str(answer.id) + "'", db.engine.connect().connection)
    df = pd.concat([df1,df2], axis=0)
    df = df[df["answer"]!="-1"]
    df = df[df["response"]!="-1"]
    ...
    task = Tasks(user_id = current_user.id, task_id = task_blueprint.id, answer = answer.id,
                place1 = question_set[0].id, place2 = question_set[1].id, place3 = question_set[2].id, 
                place4 = question_set[3].id, place5 = question_set[4].id,
